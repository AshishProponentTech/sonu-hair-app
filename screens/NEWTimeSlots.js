...
function TimeSlot({ navigation, route }) {
  ...
  const selectDateFunction = (item) => {
    // const Date = moment(item).format();
    const Date = moment(item, "M/D/YYYY").format(
      "YYYY-MM-DD[T]HH:mm:ss.SSS[Z]"
    );
    console.log("Date Check -  Step 3", Date);
    setupdatedDate(Date);
  };
          ...
          {selectStaff ? (
            <View>
              ...
              <View style={{ height: 230 }}>
                <ScrollView contentContainerStyle={styles.SlotBox}>
                  <FindSlot />
                </ScrollView>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  commonHeading: { fontSize: 17, fontWeight: "700" },
  spacing: {
    marginBottom: 10,
  },
  bottomText: {
